...
class ActivityController {
  ...
  validationSchema = {
    activityId: {
      ...
      in: ["params"],
      isInt: {
        // набо будет заменить на isString когда введем UUID вместо id
        if: (activityId) => {
          return activityId !== undefined;
        },
        errorMessage: () => {
          return i18n.__("validation.isInt", "activityId");
        },
        bail: true,
      },
      custom: {
          ...
          if (req.method === "GET") {
            ...
            return activity
              ...
              .then((is_exist) => {
                console.log(
                  is_exist,
                  "-------> is_exist activity from validationSchema"
                );

                if (is_exist.rows[0].exists !== true) {
                  console.log(
                    "Activity with activity_id = ${activityId} is not in DB (from activity_controller.js)"
                  );
                  ...
                  ); // злесь 404 как флаг, который мы проверяем в checkResult()
                }
              })
              .catch((err) => {
                if (err.error) {
                  ...
                  console.log(
                    server_error,
                    " ------------------> Server Error in validationSchema at activity_conrtoller.js"
                  );
          ...
          if (req.method === "GET") {
            ...
            return activity
              ...
              .then((is_unique) => {
                console.log(
                  is_unique,
                  "-------> is_unique activity from validationSchema"
                );

                if (is_unique.rows[0].exists == true) {
                  console.log(
                    "Activity with activity_name = ${value} is not in DB (from activity_controller.js)"
                  );
                  return Promise.reject(
                    i18n.__("validation.isUnique", `activity_name '${value}'`)
                  );
                }
              })
              .catch((err) => {
    ...
    state: {
      ...
      notEmpty: {
        errorMessage: () => {
          return i18n.__("validation.isEmpty", "state");
        },
        bail: true,
      },
      isIn: {
        ...
        errorMessage: () => {
          return i18n.__("validation.isIn", "state");
        },
        bail: true,
      },
    },
    ...
    sortBy: {
      ...
      notEmpty: {
        errorMessage: () => {
          return i18n.__("validation.isEmpty", "sortBy");
        },
        bail: true,
      },
      isArray: {
        errorMessage: () => {
          return i18n.__("validation.isArray", "sortBy");
        },
        bail: true,
      },
    },
    ...
    "sortBy.*.field": {
      ...
      notEmpty: {
        errorMessage: () => {
          return i18n.__("validation.isEmpty", "field");
        },
        bail: true,
      },
      isIn: {
        ...
        errorMessage: () => {
          return i18n.__("validation.isIn", "field");
        },
        bail: true,
      },
    },
    ...
    "sortBy.*.direction": {
      ...
      notEmpty: {
        errorMessage: () => {
          return i18n.__("validation.isEmpty", "direction");
        },
        bail: true,
      },
      isIn: {
        options: [["asc", "desc"]],
        errorMessage: () => {
          return i18n.__("validation.isIn", "direction");
        },
        bail: true,
      },
    },
    ...
    size: {
      ...
      notEmpty: {
        errorMessage: () => {
          return i18n.__("validation.isEmpty", "size");
        },
        bail: true,
      },
      isInt: {
        ...
        errorMessage: () => {
          return i18n.__("validation.isInt", "size");
        },
        bail: true,
      },
    },
    ...
    page: {
      ...
      notEmpty: {
        errorMessage: () => {
          return i18n.__("validation.isEmpty", "page");
        },
        bail: true,
      },
      isInt: {
        ...
        errorMessage: () => {
          return i18n.__("validation.isInt", "page");
        },
        bail: true,
      },
    },
    ...
    s: {
      ...
      notEmpty: {
        errorMessage: () => {
          return i18n.__("validation.isEmpty", "s");
        },
        bail: true,
      },
      isString: {
        errorMessage: () => {
          return i18n.__("validation.isString", "s");
        },
        bail: true,
      },
      isLength: {
        errorMessage: () => {
          return i18n.__("validation.isLength", "s");
        },
    ...
    if (hasError) {
      ...
      if (typeof data !== "object") {
        if (data.startsWith("404")) {
          ...
          console.log(
            not_found_error,
            ` ----> not_found_error from the ActivityController.checkResult`
          );
          ...
          console.log(
            bad_request_error,
            ` ----> bad_request_error from the ActivityController.checkResult`
          );
          res
            .status(bad_request_error.statusCode || 500)
            .json(bad_request_error);
        }
      } else {
        const server_error = data;
        console.log(
          server_error,
          ` ----> server_error from the ActivityController.checkResult`
        );
  ...
  async createActivity(req, res) {
    ...
    try {
      ...
      if (new_activity.rows[0].id) {
        ...
        console.log(
          result,
          " ----> err from createActivity function at activity_controller.js"
        );
      ...
      console.error(
        { err },
        "-----> err in createActivity function at activity_controller.js "
      );
  ...
  async updateActivity(req, res) {
    ...
    const { activity_name } = req.body;
    try {
      const updated_activity = await activity.update(
        activity_id,
        activity_name
      );
      if (updated_activity.rows.length !== 0) {
        ...
        const result = new Api404Error(
          "activity_id",
          i18n.__("validation.isExist", `activity_id = ${activity_id}`)
        );
        console.log(
          result,
          " ----> err from updateActivity function at activity_controller.js"
        );
        res.status(result.statusCode || 500).json(result);
      }
    } catch (err) {
      console.error(
        { err },
        "-----> err in updateActivity function at activity_controller.js "
      );
      res.status(err.statusCode || 500).json(err);
    }
  }
  ...
  async activateActivity(req, res) {
    try {
      ...
      if (activated_activity.rows.length == 0) {
        ...
        console.log(
          result,
          ` ----> err in activateActivity function with activity_id = ${activity_id} not exists at activity_controller.js;`
        );
        res.status(result.statusCode || 500).json(result);
      ...
      console.error(
        { err },
        "-----> err in activateActivity function at activity_controller.js "
      );
      res.status(err.statusCode || 500).json(err);
    }
  }
  ...
  async deleteActivity(req, res) {
    try {
      ...
      if (deleted_activity.rows.length !== 0) {
        ...
        console.log(
          result,
          " ----> err in deleteActivity function with activity_id = ${activity_id} not exists at activity_controller.js;"
        );
        res.status(result.statusCode || 500).json(result);
      }
    } catch (err) {
      console.error(
        { err },
        "----> err in deleteActivity function at activity_controller.js "
      );
      res.status(err.statusCode || 500).json(err);
    }
  }
  ...
  async retrieveSingleActivity(req, res) {
    ...
    try {
      ...
      if (get_activity.rows.length !== 0) {
        ...
        console.log(
          result,
          ` -----> err in retrieveSingleActivity function with activity_id = ${activity_id} not exists at activity_controller.js;`
        );
        res.status(result.statusCode || 500).json(result);
      }
    } catch (err) {
      console.error(
        { err },
        "---->err in retrieveSingleActivity function at activity_controller.js "
      );
      res.status(err.statusCode || 500).json(err);
    }
  }
  ...
  async retrieveMultipleActivities(req, res) {
    try {
      ...
      const get_activities = await activity.findAll({
        state,
        sortBy,
        limit,
        offset,
      ...
      if (get_activities[0].rows.length !== 0) {
        ...
        const result = new Api404Error(
          "activity_name",
          i18n.__("validation.isExist", `${s}`)
        );
        console.log(
          result,
          ` -----> err in retrieveMultipleActivities function  with  ${s} not exists at activity_controller.js;`
        );
        res.status(result.statusCode || 500).json(result);
      }
    } catch (err) {
      console.error(
        { err },
        "---->err in retrieveMultipleActivities function at activity_controller.js "
      );
      res.status(err.statusCode || 500).json(err);
    }
  }
    ...
    try {
      ...
      if (popular_activities.rows.length !== 0) {
        ...
        const result = new Api404Error(
          ...
          i18n.__("validation.isExist", "getPopularActivities")
        );
        console.log(
          result,
          ` -----> err in retrievePopularActivities function  at activity_controller.js;`
        );
        res.status(result.statusCode || 500).json(result);
      }
    } catch (err) {
