...
class UsersModel {
  async isExistUserId(user_id) {
    try {
      ...
      console.log(
        err,
        `-----> err in isExist function with user_id = ${user_id}  at  user_model.js`
      );
  ...
  async isExistRoles(roles) {
    try {
      ...
      const roles_id = await db.query(
        `SELECT id AS exists FROM roles ORDER BY id;`
      ); // Проверуку убрать в контороллер в валидациию
      let list_roles_id = [];
      ...
      console.log(
        err,
        `-----> err in isExist function with roles = ${roles}  at  user_model.js`
      );
      // console.log(err.message, '-----> err.message')
  ...
  async isUniqueEmail(email) {
    try {
      ...
      console.log(
        err,
        `-----> err in isUnique function with email = ${email}  in user_model.js`
      );
      // console.log(err.message, '-----> err.message')
  ...
  async isUniqueProfilId(profile_id) {
    try {
      ...
      console.log(
        is_unique.rows,
        " -----> is_unique.rows in  isUniqueProfilId function from user_model.js"
      );
      return is_unique;
    } catch (err) {
      console.log(
        err,
        `-----> err in isUniqueProfilId function with profile_id = ${profile_id}  in user_model.js`
      );
      // console.log(err.message, '-----> err.message')
  ...
  async isUniqueCombination(profile_id, service) {
    try {
      ...
      console.log(
        is_unique.rows,
        " -----> is_unique.rows in  isUniqueCombination function from user_model.js"
      );
      return is_unique;
    } catch (err) {
      console.log(
        err,
        `-----> err in isUniqueCombination function with profile_id = ${profile_id}  in user_model.js`
      );
      // console.log(err.message, '-----> err.message')
  ...
  async create(
    ...
    try {
      ...
      if (roles_id.length > 1) {
        console.log(
          new_role[0].rows,
          " ----> new_role[0].rows at serviceprovider_model.js"
        );
        return { new_user: new_user, new_role: new_role };
      } else {
        console.log(
          new_role.rows,
          " ----> new_role.rows at serviceprovider_model.js"
        );
        return { new_user: new_user, new_role: [new_role] };
      }
    } catch (err) {
      console.log(
        err,
        `-----> err in create function with email = ${email}  at user_model.js`
      );
      // console.log(err.message, '-----> err.message')
  ...
  async update(
    ...
    try {
      ...
      console.log(
        err,
        `-----> err in update function with user_id = ${user_id}  at user_model.js`
      );
      // console.log(err.message, '-----> err.message')
      throw new Api500Error("user_id", `${err.message}`);
    }
  ...
  async activate(user_id, active) {
    try {
      const activated_person = await db.query(
        `UPDATE users
            SET active = $2
            WHERE id = $1
            RETURNING *;`,
      ...
      console.log(
        err,
        `-----> err in activate function with user_id = ${user_id}  at user_model.js`
      );
      // console.log(err.message, '-----> err.message')
      throw new Api500Error("user_id", `${err.message}`);
    }
  ...
  async delete(user_id) {
    try {
      const deleted_person = await db.query(
        `DELETE FROM users 
      ...
      console.log(
        err,
        `-----> error  in delete function with user_id = ${user_id}  at user_model.js`
      );
      // console.log(err.message, '-----> err.message')
      throw new Api500Error("user_id", `${err.message}`);
    }
    ...
    try {
      ...
      if (found_user.rows.length == 0) {
        ...
        console.log(
          err,
          user,
          "test (err, user) Error 401 (User not found) user_model.js -84"
        );
        callback(err, user);
        ...
        console.log(
          err,
          user,
          "test (err, user) (User found) user_model.js -90"
        );
        callback(err, user);
      }
    } catch (err) {
      console.log(
        err,
        `-----> err  in findOne function with username = ${username}  at user_model.js`
      );
      // console.log(err.message, '-----> err.message')
  ...
  async findByProfileId(profile) {
    try {
      ...
      console.log(
        found_user.rows,
        "found_user.rows test findByProfileId from user_model.js"
      );
      ...
      if (found_user.rows.length == 0) {
        ...
        console.log(
          err,
          user,
          "test (err, user) Error 401 (User not found) user_model.js"
        );
        return found_user;
      } else if (found_user.rows[0]) {
        const err = null;
      ...
      console.log(
        err,
        `-----> err  in findByProfileId function with profile_id = ${profile.profile_id}  at user_model.js`
      );
      // console.log(err.message, '-----> err.message')
      throw new Api500Error("profile_id", `${err.message}`);
    }
  ...
  async findOneWithRoles(user_id) {
    try {
      const get_user =
        await db.query(`SELECT id, AS user_id, active, email, phone, first_name, last_name, patronymic, dob, role_id 
      ...
      console.log(
        err,
        `-----> err  in findOneWithRoles function with user_id = ${user_id}  at user_model.js`
      );
      // console.log(err.message, '-----> err.message')
      throw new Api500Error("user_id", `${err.message}`);
    }
  ...
  async findAll({ state, sortBy, limit, offset, s }) {
    ...
    try {
      ...
      const query_count =
        "SELECT COUNT(id) AS count FROM users LEFT JOIN users_roles ON users.id = users_roles.user_id  ";
      const and = "AND ";
      ...
      for (let i = 0; i < get_users[0].rowCount; i += 1) {
        if (
          get_users[0].rows[i] &&
          get_users[0].rows[i + 1] &&
          get_users[0].rows[i].id !== get_users[0].rows[i + 1].id
        ) {
          ...
          get_users[0].rows[i] &&
          get_users[0].rows[i + 1] === undefined
        ) {
          roles.push(get_users[0].rows[i].role_id);
          let user = {
            user_id: get_users[0].rows[i].id,
